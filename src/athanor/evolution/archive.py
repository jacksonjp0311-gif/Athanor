...
class MapElitesArchive:
    ...
    def descriptor(self, cand) -> Tuple[int,int]:
        ...
        dn = float(cand.tags.get(""delta_norm"", 0.0)) if cand.tags else 0.0
    ...
    def add(self, cand) -> bool:
        ...
        if (cur is None) or (cand.score_f > cur.best_f):
            self.cells[key] = ArchiveCell(best_id=cand.id, best_f=float(cand.score_f), meta={
                ""h7"": float(cand.dphi.h7) if cand.dphi else 0.0,
                ""delta_norm"": float(cand.tags.get(""delta_norm"", 0.0)) if cand.tags else 0.0,
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {""filled"": int(len(self.cells)), ""mean_best_f"": float(fs.mean()), ""max_best_f"": float(fs.max())}
